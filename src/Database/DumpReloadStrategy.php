...
final class DumpReloadStrategy implements ReloadStrategyInterface
{
    ...
    public function needsBootstrap(DatabaseReloaderConnection $connection): bool
    {
        if ($connection->driver() === 'sqlite') {
            $testPath = $connection->test->path ?? '';
            if ($testPath === '') {
            ...
            return !is_file($testPath);
        }

        return !is_file($this->dumpFilePath($connection));
    }
    ...
    private function buildMariaDbDropCreate(Dsn $dsn): Command
    {
        ...
        $escapedDatabaseName = str_replace('`', '``', $database);
    ...
    {
        $cmd = ['mysqldump', '--no-data', '--routines', '--events', '--triggers', '--single-transaction', '--skip-ssl'];
    ...
    private function terminateMariaDbConnections(Dsn $dsn): void
    {
        $database = $dsn->database ?? '';
        if ($database === '') {
            throw new DatabaseReloaderException('MariaDB test database name is empty.');
        ...
        $listResult = $this->runner->run($this->buildMariaDbCommand(['-N', '-e', $listSql], $dsn));
        if (!$listResult->isOk()) {
            throw new DatabaseReloaderException(sprintf(
                'Failed to fetch MariaDB sessions for test database "%s". %s',
                $database,
                $listResult->stderr !== '' ? $listResult->stderr : $listResult->stdout,
            ));
        }

        $processIds = $this->parseMariaDbProcessIds($listResult->stdout);
        if ($processIds === []) {
            return;
        }

        foreach ($processIds as $processId) {
            $killSql     = sprintf('KILL %d;', $processId);
            $killCommand = $this->buildMariaDbCommand(['-e', $killSql], $dsn);
            $killResult  = $this->runner->run($killCommand);

            if ($killResult->isOk() || $this->isIgnorableMariaDbKillError($killResult)) {
                continue;
            }

            throw new DatabaseReloaderException(sprintf(
                'Failed to terminate MariaDB session %d for test database "%s". %s',
                $processId,
                $database,
                $killResult->stderr !== '' ? $killResult->stderr : $killResult->stdout,
            ));
        }
    }
     ...
     * @return list<int>
     */
    private function parseMariaDbProcessIds(string $stdout): array
    {
        $processIds = [];
        foreach (explode("\n", $stdout) as $line) {
            $value = trim($line);
            ...
            $processIds[] = (int) $value;
        }

        return $processIds;
    }

    private function isIgnorableMariaDbKillError(CommandResult $result): bool
    {
        $output = $result->stderr !== '' ? $result->stderr : $result->stdout;

        return str_contains($output, 'ERROR 1094') || str_contains($output, 'Unknown thread id');
    }
}
